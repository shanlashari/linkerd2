...
This release adds support for retrying HTTP/2 messages with small (<64KB)
bodies, allowing the proxy to properly buffer and message bodies when responses
are classified as a failure.
...
* Fixed an issue where `linkerd check --proxy` could incorrectly report
  out-of-date proxy versions caused by incorrect formatting (thanks @aryan9600!)
* Added support for the proxy to retry HTTP/2 requests with message bodies
  <= 64KB
* Modified the proxy's controller stack to create new client connections for
  on-demand
